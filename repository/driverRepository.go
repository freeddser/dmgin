...
import (
	"encoding/json"
	"github.com/freeddser/dmgin/flags"
	"github.com/freeddser/dmgin/models"
	"strconv"
)
...
func (repo *driverRepository) GetDriverByID(driverID int64) (models.Driver, error) {
	...
	if len(cacheData) < 1 {
		err := repo.db.QueryRow(getDriverByID, driverID, flags.ACTIVE).Scan(&driver.DriverID, &driver.DriverName, &driver.DriverMobile, &driver.VehicleID, &driver.VehicleNo, &driver.Status, &driver.CreatedAt, &driver.UpdatedAt)
		if err != nil {
			log.Error(err)
			return driver, err
		}
